...
export default function CommandCenterPage() {
  ...
  async function markCompleted(customer: any) {
    ...
    const { error } = await supabase
      .from("customers")
      .update({ has_photo: false, status: "BOT_ACTIVE" })
      .eq("id", customer.id);
  ...
  async function handleSlotClick(day: string, time: string) {
    ...
    // Open the modal immediately with slot data
                        ...
                        {days.map((d) => {
                          ...
                          return (
                            <button
                              key={`${d}-${t}`}
                              disabled={isBlocked}
                              onClick={() => isConfirmed && handleSlotClick(d, t)}
                ...
                {customers.length === 0 ? (
                  ...
                  customers.map((customer) => {
                    const formattedTime = customer.last_seen_at
                      ...
                      : null;
                    const name = customer.name || customer.first_name || (customer.instagram_id ? `@${customer.instagram_id}` : "Unknown Patient");
                    const hasPhoto = Boolean(customer.last_dental_image);

                ...
                {bizOwner?.instagram_username ? (
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const url = `https://www.facebook.com/dialog/oauth?client_id=${process.env.NEXT_PUBLIC_META_APP_ID}&redirect_uri=${encodeURIComponent(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/instagram/callback`)}&scope=instagram_basic,instagram_manage_messages,pages_show_list,pages_read_engagement&response_type=code&state=${encodeURIComponent(state)}&auth_type=rerequest`;
